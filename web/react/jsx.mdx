...
* Wir können ganze JavaScript-Expressions in den `{}` schreiben, z.B. um Variablen zu referenzieren, Arrays oder Objekte zu erstellen und die [`.map()`-Methode](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map), den [Ternary-Operator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Conditional_operator), etc. nutzen
* Statements sind nicht erlaubt, z.B. `if/else`, `for` und `switch`
* Ein Stück JSX darf nur ein Root-Element beinhalten. Wenn man mehr braucht, sollte man [`<>`](/web/react/komponenten/fragments) benutzen
