...
Die `map()`-Methode funktioniert ähnlich wie die [`forEach()`-Methode](/web/javascript/arrays/iteration-uber-arrays#foreach). Sie geht durch jedes Element im Array durch und führt eine bestimmte Callback-Methode aus.
